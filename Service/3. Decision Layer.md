...
본 모듈은 **Spatio-Temporal Graph Neural Network (ST-GCN)**와 **Transformer** 아키텍처를 결합하여 선수의 동작과 생체 신호를 분석하며, **Personalized Digital Twin(개인화 디지털 트윈)** 기술을 통해 선수 개개인의 고유한 신체 특성을 반영한 정밀한 위험도(Risk Score) 산출을 수행한다.

---

## 2. AI 모델 아키텍처 (AI Model Architecture)

본 시스템은 다중 모달(Multi-modal) 데이터를 처리하기 위해 3가지 전문화된 뉴럴 네트워크의 앙상블(Ensemble) 구조를 채택한다.
...
THz 라디오메트리 신호와 반사 계수 데이터를 결합하여 심부 온도와 탈수 지수를 산출한다.
Algorithm: Physics-Informed Neural Networks (PINN) 및 MLP-Mixer.
Input: * Passive Signal: $0.1 \sim 1.5 \text{ THz}$ 대역의 방사 전압($V_{out}$). Active Signal: THz 반사 계수($\Gamma$) 및 위상 변이($\Delta \phi$).Context Data: 기온, 습도, 선수의 기초 대사량($Q_m$).Operation:Bio-heat Solver Layer: Pennes 생체 열 방정식을 손실 함수(Loss Function)에 포함하여, 측정된 피부 온도($T_s$)와 혈류량($\omega_b$)으로부터 **심부 온도($T_b$)**를 역계산한다.Sweat Duct Analyzer: 땀샘의 나선형 안테나 공명 주파수($f_r$) 이동을 추적하여 발한량(Sweating Rate)을 수치화한다.Output: Core Body Temperature ($^\circ\text{C}$), Hydration Level (%).
---
...
최종 위험 점수 $R_{total}$은 세 가지 요인의 가중 합으로 계산된다.

$$R_{total}(t) = w_1 \cdot R_{kin}(t) + w_2 \cdot R_{trem}(t) + w_3 \cdot R_{vit}(t)$$

* **$R_{kin}$ (운동학적 위험도):** ST-GCN 출력. 관절 꺾임 각도 $\theta$가 임계치 $\theta_{th}$를 초과하는 정도.
    * $R_{kin} = \text{Sigmoid}(\alpha (\theta_{current} - \theta_{th}))$
* **$R_{trem}$ (떨림 위험도):** Transformer 출력. 근육 경련 확률 ($0 \sim 1$).
* **$R_{vit}$ (바이탈 위험도):** 심박/호흡의 비정상 변동성.
* **$w_1, w_2, w_3$:** 가중치 (상황에 따라 동적 조정, $\sum w_i = 1$).

* 최종 위험 점수 $R_{total}$은 운동학적 위험과 생체 내부 위험의 가중 합으로 계산된다.$$R_{total}(t) = w_1 \cdot R_{kin}(t) + w_2 \cdot R_{trem}(t) + w_3 \cdot R_{vit}(t)$$$R_{kin}$ (운동학적 위험도): ST-GCN 출력값. 관절의 굴곡/신전 각도가 해부학적 임계치($\theta_{th}$)를 초과하는 정도를 시그모이드 함수로 정규화한다. +1$R_{trem}$ (떨림 위험도): CNN-Transformer 출력값. 근육 피로 누적에 따른 8~12Hz 대역의 생리학적 떨림 진폭을 반영한다. +1$R_{vit}$ (바이탈 위험도): 심박/호흡 변동성과 함께 심부 체온 및 탈수 지수를 결합한 복합 지표이다.$$R_{vit}(t) = \beta_1 \cdot R_{temp}(t) + \beta_2 \cdot R_{hyd}(t) + \beta_3 \cdot R_{cardio}(t)$$$R_{temp}$: PINN 모델로 역산된 심부 온도 $T_b$가 위험 임계치($39.5\text{°C}$)에 근접하는 지수이다. $R_{hyd}$: THz 반사 계수($\Gamma$) 변화를 통해 산출된 체내 수분 손실률이 2%를 초과하는 정도이다. +1
...
Decision Layer는 단순한 규칙 기반(Rule-based) 시스템을 넘어, **Deep Learning**과 **Digital Twin** 기술을 결합하여 상황 맥락(Context)을 이해하는 지능형 시스템이다.
특히 **ST-GCN**을 통한 구조적 분석과 **Transformer**를 통한 시계열적 이상 감지의 결합은, 인간 심판이 인지할 수 없는 0.1초 찰나의 부상 징후를 정확히 포착하여 스포츠 안전의 새로운 표준을 제시한다.
...
6. "Morphological study of sweat ducts for the investigation of terahertz waves interaction with human skin" (Nature 등)
