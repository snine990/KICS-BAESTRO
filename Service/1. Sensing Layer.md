...
2.4. THz 복소 유전율 기반 수분 민감도 모델 (THz Complex Permittivity)
테라헤르츠(THz) 파동은 물 분자의 회전 및 진동 에너지 준위와 일치하여 수분에 극도로 민감하게 반응한다.
Complex Permittivity & Reflection:피부 표면의 수분 함량에 따라 피부의 복소 유전율($\epsilon_{skin}$)이 변화하며, 이는 전파의 반사 계수($\Gamma$)를 결정한다.$$\Gamma = \frac{\sqrt{\epsilon_{skin}} - \sqrt{\epsilon_{air}}}{\sqrt{\epsilon_{skin}} + \sqrt{\epsilon_{air}}}$$Emissivity-Hydration Correlation:
논문에 따르면 THz 대역에서 피부의 방사율(Emissivity, $\epsilon$)은 약 0.4에서 0.9까지 변화하며, 이는 피부 하부의 생체수(Biological Water) 유전율 감소에 기인한다. Helical Antenna Effect (땀샘 공명):인체의 땀샘은 나선형(Helical) 구조를 띠며, $0.1$ ~ $1 \text{ THz}$ 대역에서 나선형 안테나로 작동한다. 땀샘 내부가 수분으로 채워지는 정도에 따라 공명 주파수($f_r$)가 이동하는 특성을 이용해 미세 발한량을 측정한다.2.5. 수동형 THz 라디오메트리 및 생체 열 역계산 (Passive THz Radiometry)6G 기지국 센서(TeraFET)가 인체에서 자연 방출되는 열복사 신호를 포착하여 온도를 산출하는 원리이다. +1Planck’s Law & Rayleigh-Jeans Approximation:
THz 대역($f < 6.15 \text{ THz}$)에서 인체의 방사 에너지 강도($I$)는 절대 온도($T$)에 선형 비례한다. $$I(f, T) \approx \frac{2f^2 k_B T}{c^2}$$Pennes’ Bio-heat Equation (심부 온도 역계산):측정된 피부 온도($T_s$)와 6G 도플러 센싱으로 얻은 혈류량($\omega_b$) 데이터를 결합하여 내부 심부 온도($T_b$)를 산출한다.$$\rho c \frac{\partial T}{\partial t} = \nabla \cdot (k \nabla T) + \omega_b \rho_b c_b (T_b - T) + Q_m$$$\omega_b$ (Blood Perfusion): 6G ISAC의 혈류 속도 센싱 값 대입.$T_b$ (Core Temperature): 최종 역산 대상인 타겟 데이터.
---
