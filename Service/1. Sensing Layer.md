...
* **Measurement Metrics:** **TeraFET 검출기**를 통해 포착된 $0.1 \sim 1.5 \text{ THz}$ 대역의 광대역 열복사(Thermal Radiation) 전압 신호.
* **Threshold Settings:**
    * **Warning (주의):** $38.5^\circ\text{C}$ 이상 (피로 급증, 주의력 저하 시작).
    * **Critical (위험):** $39.5^\circ\text{C}$ 이상 (열사병 징후, 즉각적인 경기 중단 권고).
* **Biomarker Value:** 외부 온도계나 열화상 카메라가 감지하지 못하는 **'내부 열 축적(Internal Heat Accumulation)'**을 감지하여, 고온 환경에서의 급성 열사병(Heat Stroke)을 예방.
...
5.  "Passive Detection and Imaging of Human Body Radiation Using an Uncooled Field-Effect Transistor-Based THz Detector" (Sensors 2020): 상온 TeraFET 소자를 이용한 인체 방사 에너지 감지 및 $4.4\text{ K}$ NETD 정밀도 실증.
   핵심 근거: 비접촉식 체온 모니터링의 하드웨어적 타당성 확보.
"Morphological study of sweat ducts for the investigation of terahertz waves interaction with human skin" (Nature 등): 인체 땀샘의 나선형 안테나 모델링 및 THz 수분 감지 원리.핵심 근거: 센서 없는 탈수 상태 및 발한량 측정의 물리적 모델링.
"Pennes' Bio-heat Equation in Biological Systems" (Journal of Applied Physics): 외부 피부 온도를 통한 내부 심부 온도 역계산 알고리즘의 수학적 토대.
