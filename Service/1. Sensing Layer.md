...
### D. 피부 수분도 및 발한 지수 (Hydration & Sweat Index)
* **Definition:** 피부 표면의 수분 함량($\%$) 및 단위 시간당 땀 배출량(mg/min/cm²).
* **Measurement Metrics:**
    * **THz Reflection Coefficient:** 피부 유전율 변화에 따른 반사 계수($\Gamma$)의 진폭 변화.
    * **Resonance Phase Shift:** 땀샘(Sweat Duct)이 차오를 때 발생하는 안테나 공명 신호의 위상 변이.
* **Biomarker Value:** 선수의 체중 대비 2% 수분 손실(탈수 임계점)을 실시간으로 예측하여, 근육 경련 및 운동 수행 능력 저하(Performance Drop)를 선제적으로 방어.

### E. 실시간 심부 체온 (Real-time Core Body Temperature)
* **Definition:** 피부 온도가 아닌, 신체 내부 장기 및 혈액의 온도(Core Temp).
* **Measurement Metrics:**
    * Thermal Signal: TeraFET 검출기를 통한 $0.1 \sim 1.5 \text{ THz}$ 대역의 방사 전압($V_{out}$) 기반 온도 분포 맵입니다.
    * Hemorrhage Index: 타격 지점의 국소적 방사율($\epsilon$) 급변 및 혈류 도플러 신호의 비정상적인 에너지 밀도 변화(조직 내 혈액 고임)를 감지합니다.
* **Threshold Settings:**
    * **Warning (주의):** 심부 온도 $38.5^\circ\text{C}$ 이상 (피로 급증, 주의력 저하 시작) 또는 타격 부위의 국소 온도 $2^\circ\text{C}$ 이상 급상승 시(미세 혈관 파열 및 피하 출혈 시작 징후).
    * **Critical (위험):** 심부 온도 $39.5^\circ\text{C}$ 이상 (열사병 징후, 즉각적인 경기 중단 권고) 또는 특정 부위의 방사율 지표가 '내부 대량 출혈' 임계값을 돌파할 시(장기 손상 및 대형 혈종 발생 위험).
* **Biomarker Value:** 격투기 시합 중 겉으로 드러나지 않는 신체 내부의 과열과 출혈 상태를 실시간으로 포착하여, '세컨드 임팩트 신드롬' 및 주요 장기 부전을 선제적으로 방어합니다.
...
* Radiometric Calibration: 하드웨어 자체의 열 잡음을 제거하고, 수신된 전압 신호($V_{out}$)를 절대 복사 에너지로 변환.
* Water Vapor Compensation: 대기 중 수증기에 의한 THz 감쇠 신호를 실시간 기상 데이터(습도)를 바탕으로 보정하여 신뢰도 확보.
...
* Emissivity Mapping (방사율 맵 생성): Step 3의 반사 계수($\Gamma$) 데이터를 기반으로 피부의 실시간 방사율($\epsilon$)을 계산.
*    공식: $\epsilon = 1 - |\Gamma|^2$ (에너지 보존 법칙 기반 역산).
*    Spectrogram Analysis (땀샘 공명): STFT 결과에서 땀샘의 나선형 안테나 공명 주파수($f_r$) 대역을 하이라이트하여 발한량 지표 추출.
*    Thermal Radiometry Image: $V_{out}$ 데이터를 레일리-진스 법칙에 대입하여 2D/3D 온도 분포 맵(Brightness Temperature Map) 생성.
...
| **Thermal Precision** | N/A (적외선 한계) | **4.4 K (NETD) @ 300K** | 상온 TeraFET 실증 성능 |
| **Sensing Bandwidth** | < 1 GHz | **0.1 THz ~ 1.5 THz** | 수분 및 체온 복합 감지 가능 |
| **Hydration Sensitivity** | Hydration Sensitivity | **Reflectivity $\Delta \approx 0.5$** | 탈수 상태(Emissivity 0.4~0.9) 식별 |
| **Core Temp Accuracy** | $\pm 1.0\text{°C}$ (추정) | **< $\pm 0.2\text{°C}$ (역계산)** | Bio-heat 방정식 기반 정밀 역산 |
...
* **Weak Passive Signal (미약한 열 복사 신호):** 인체에서 방출되는 THz 에너지는 매우 미약하여 배경 잡음에 묻히기 쉬움.
    * $\rightarrow$ Solution: 상온 TeraFET 검출기와 고이득 로그-스파이럴 안테나를 결합하여 $40\text{ mA/W}$ 급의 높은 응답도(Responsivity) 확보.
* **Emissivity Uncertainty (방사율 불확실성):** 개인별 피부 특성 및 수분 상태에 따라 방사율이 변하여 온도 측정 오차 발생.
    * $\rightarrow$ Solution: 6G 능동 센싱으로 측정된 **반사 계수($\Gamma$)** 를 실시간 대입하여 방사율($\epsilon = 1 - |\Gamma|^2$)을 보정하는 Self-Calibration 알고리즘 적용.
* **Atmospheric Absorption (수증기 감쇠):** THz 대역은 대기 중 수증기에 의해 신호가 크게 감쇄됨.
    * $\rightarrow$ Solution: 6G 기지국 간 전송 데이터(CSI)를 분석하여 경로 상의 습도를 실시간 측정하고, 이를 Radiometric 파이프라인의 보정 상수로 자동 반영.
...
5.  **"Passive Detection and Imaging of Human Body Radiation Using an Uncooled Field-Effect Transistor-Based THz Detector" (Sensors 2020):** 상온 TeraFET 소자를 이용한 인체 방사 에너지 감지 및 $4.4\text{ K}$ NETD 정밀도 실증. 비접촉식 체온 모니터링의 하드웨어적 타당성 확보.
6.  **"Morphological study of sweat ducts for the investigation of terahertz waves interaction with human skin" (Nature 등):** 인체 땀샘의 나선형 안테나 모델링 및 THz 수분 감지 원리.핵심 근거: 센서 없는 탈수 상태 및 발한량 측정의 물리적 모델링.
7.  **"Pennes' Bio-heat Equation in Biological Systems" (Journal of Applied Physics):** 외부 피부 온도를 통한 내부 심부 온도 역계산 알고리즘의 수학적 토대.
